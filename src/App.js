...
const AuthPage = ({ onLogin, onSignup }) => {
    ...
    return (
        <div className="auth-container">
            ...
            <div className="auth-complex-shapes">
                <div className="auth-complex-shape"></div>
                <div className="auth-complex-shape"></div>
                <div className="auth-complex-shape"></div>
                <div className="auth-complex-shape"></div>
                <div className="auth-complex-shape"></div>
            ...
            {/* Advanced particles */}
            <div className="auth-advanced-particles">
                <div className="auth-advanced-particle"></div>
                <div className="auth-advanced-particle"></div>
                <div className="auth-advanced-particle"></div>
                <div className="auth-advanced-particle"></div>
                <div className="auth-advanced-particle"></div>
            ...
            <div className="auth-card">
                <div className="auth-header">
                    ...
                    <p>{isSignUp ? 'Create an account to get started' : 'Welcome back! Please sign in to your account'}</p>
                </div>
                <div className="auth-card-content">
                    <form onSubmit={handleSubmit} className="auth-form">
                        ...
                        <div className="form-group">
                            ...
                            <div className="input-wrapper">
                                <div className="input-icon">{Icons.info}</div>
                                <input
                                    type="password"
                                    name="password"
        ...
        <div className="auth-container">
            ...
            <div className="auth-card">
                <div className="auth-header">
                    ...
                    <p>Connect your wallet to complete your account setup</p>
                </div>
                <div className="auth-card-content">
                    <div className="wallet-content">
                        <div className="wallet-info">
                            ...
                            <h3>MetaMask</h3>
                            <p>Connect using your MetaMask wallet to securely link it to your account</p>
                        </div>
                        <button
                        ...
                        >
                            {loading ? <Loader /> : (
                                <>
                                    <span className="wallet-button-icon">{Icons.metamask}</span>
...
export default function App() {
    ...
    // Verify wallet when dashboard loads
    useEffect(() => {
        if (isAuthenticated && jwtToken && account) {
            verifyWalletConnection();
        }
    }, [isAuthenticated, jwtToken, account]);
    ...
    // Verify wallet connection matches the one in JWT
    const verifyWalletConnection = async () => {
        if (!account || !jwtToken) return;
        try {
            // Decode JWT to get collegeAddress
            ...
            if (account.toLowerCase() !== storedWallet.toLowerCase()) {
                ...
                setStatus({
                    type: 'error',
                    message: `Wallet Mismatch! Please switch to your linked account (${storedWallet}) in MetaMask to continue.`
                });
            } else {
    ...
    const handleLogin = async (username, password) => {
        try {
            ...
            // Save JWT and set authenticated state
            localStorage.setItem('jwtToken', data.token);
            setJwtToken(data.token);
            setIsAuthenticated(true);
            
            // If user has a linked wallet, set it
            if (data.collegeAddress) {
                setAccount(data.collegeAddress);
            }
        } catch (error) {
            throw error;
        }
        ...
        try {
            ...
            try {
                ...
                const tx = await contract.createCertificateWithProof(
                    ...
                    dateOfIssuing, 
                    contentHash, 
                    accreditationProof.collegeName, 
                    accreditationProof.validUntil, 
                    accreditationSignature
                );
    ...
    const renderAccreditationContent = () => {
        switch (requestStatus) {
            case 'pending': return (
                <div>
                    <p>Your request is pending administrator approval.</p>
                    <button onClick={() => handleCheckStatus()} className="btn btn-secondary" disabled={loadingStates.checkStatus}>
                        {loadingStates.checkStatus ? <Loader /> : 'Check Status'}
                    </button>
                </div>
            );
            case 'approved': return (
                <div>
                    <p>Your accreditation is approved! You can now proceed to the next step.</p>
                </div>
            );
            case 'rejected': return (
                <div>
                    <p>Your request was denied. Please contact the administrator for more information.</p>
                </div>
            );
            default: return (
                <div className="card-content">
                    <p>Enter your college's name to request accreditation from the administrator.</p>
                    <IconInput 
                        icon={Icons.accreditation} 
                    ...
                    <button 
                        ...
                        disabled={!account || !collegeName || loadingStates.request}
                    >
                        {loadingStates.request ? <Loader /> : 'Request Accreditation'}
                    </button>
                </div>
            );
        }
    ...
    return (
        <div className="dashboard-layout">
            ...
            <main className="main-content">
                ...
                {walletMismatch && (
                    <div className="wallet-mismatch-warning">
                        {Icons.error}
                        <div>
                            <strong>Wallet Mismatch!</strong>
                            <p>Please switch to your linked account in MetaMask to continue.</p>
                        </div>
                    </div>
                )}
                ...
                <div className="card-grid">
                    ...
                    <section className={`card ${requestStatus !== 'approved' ? 'disabled' : ''}`}>
                        ...
                        <div className="card-content">
                            ...
                            <IconInput 
                                icon={Icons.rollNo} 
                                placeholder="Roll Number (e.g., A-007)" 
                                value={rollNo} 
                            ...
                            <IconInput 
                                ...
                                type="date" 
                                value={dateOfIssuing} 
                                onChange={e => setDateOfIssuing(e.target.value)} 
                            />
                            <div style={{display: 'flex', gap: '1rem', flexWrap: 'wrap'}}>
                                <button 
                                    onClick={handleCheckDuplicate} 
                                    className="btn btn-secondary" 
                                    disabled={!rollNo || loadingStates.checkDuplicate || walletMismatch}
                                >
                                    {loadingStates.checkDuplicate && <Loader />}Check Duplicate
                                </button>
                                <button 
                                    onClick={handleIssueCertificate} 
                                    className="btn btn-primary" 
                                    disabled={!canIssue || loadingStates.issue || walletMismatch}
                                >
                                    {loadingStates.issue && <Loader />}Issue On-Chain
                                </button>
                            </div>
                            {issuedCertificateId && (
                                <div className="certificate-id-display" style={{ 
                                    marginTop: '1rem', 
                                    padding: '0.75rem', 
                    ...
                    <section className="card full-width">
                        ...
                        <div className="card-content">
                            <div className="verify-group">
                                <IconInput 
                                    icon={Icons.verify} 
                                    placeholder="Enter Certificate ID to verify its authenticity" 
                                    value={verifyId} 
                                ...
                                >
                                    {loadingStates.verify ? <Loader /> : 'Verify Certificate'}
                                </button>
                            </div>
                            {verifyResult && (
                                <div className="verify-result">
                                    ...
                                    {verifyResult.certificate && (
                                        <pre>{JSON.stringify(verifyResult.certificate, null, 2)}</pre>
                                    )}
                                </div>
                            )}
                        </div>
